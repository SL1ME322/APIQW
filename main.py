from fastapi import FastAPI, Query, Depends, HTTPException, status, BackgroundTasks, Response
...
smtp_handler = logging.handlers.SMTPHandler(
    ...
    fromaddr='bioironman919@gmail.com',
    toaddrs=['isip_d.a.halekov@mpt.ru'],
...
def get_user(db, username: str):
    ...
    row = cursor.fetchone()
    if row:
        user_dict = {"ID_User": row.ID_User, "Login": row.Login,
                     "hashed_password": row.Password, "Avatar": row.Avatar,
...
def authenticate_user(db, username: str, password: str):
    ...
    if not verify_password(password, user.hashed_password):
        return False
    logger.info(f"User: {username} authenticated sucessfully")
    return user
...
async def login_for_access_token(
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail="Неверный логин или пароль", headers={"WWW-Authenticate": "Bearer"})
...
async def root(request: Request):
    ...
    logger.info(f"Request to endpoint: {endpoint} sucessful")
    return {"message ": "hi"}
...
# async def send_log_file_via_email():
#     msg = MIMEText('wqwqwqwdxfdfdgd', "html")
#     msg['Subject'] = "Error in user registration"
#     msg['From'] = 'bioironman919@gmail.com'
#     msg['To'] = 'bioironman919@mail.ru'
#     port = 465

#     try:
#         async with aiosmtplib.SMTP_SSL(hostname="mail.privateemail.com", port=port) as smtp:
#             await smtp.login('bioironman919@mail.ru', '$Ijj2VOooQR8VvmvTr~mfFy@b')
...
async def register_new_user(user: UserInDB, response: Response):
    try:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO [dbo].[User](Login, Password, Avatar, Registration_Date, Description) "
                       "VALUES (?, ?, ?, ?, ?)",
                       user.Login, get_password_hash(user.hashed_password), image_url_to_base64(
                           user.Avatar), user.Registration_Date, user.Description
                       )
        conn.commit()
        logger.info(f"Status Code 200: Registration sucessful")
    except Exception as e:
        port = 587
        smtp_server = "smtp.gmail.com"
        sender_email = "bioironman919@gmail.com"
        receiver_email = "isip_d.a.halekov@mpt.ru"
        password = "lnao pbtm jwbs hhxp"
        message = """\
        Subject: 500 code status
        Encountered error in registration."""
...
async def getEstatesOnMainPage(page: int = Query(default=1, description="Номер страницы", ge=1),
                               items_per_range: int = Query(
                                   default=1, description="Кол-во элементов на странице", ge=1),
